...
def esAlertsSummary(begindateUTC=None, enddateUTC=None):
    ...
    try:
        ...
        # q=S().es(urls=['http://{0}:{1}'.format(options.esserver,
        # options.esport)]).query(_type='alert').filter(utctimestamp__range=
        # [begindateUTC.isoformat(),enddateUTC.isoformat()])
        # f=q.facet_raw(alerttype={"terms" : {"script_field" : "_source.type",
        # "size" : 500}})

        # get all alerts
        # q= S().es(urls=['http://{0}:{1}'
        # .format(options.esserver,options.esport)])
        # .query(_type='alert')
        q = S().es(urls=list('{0}'.format(s)
                             for s in options.esservers)).query(_type='alert')
        # create a facet field using the entire 'type' field  (not the sub
        # terms) and filter it by date.
        f = q.facet_raw(
            alerttype={"terms": {"script_field": "_source.category"},
                       "facet_filter": {'range': {'utctimestamp':
                                       {'gte': begindateUTC.isoformat(),
                                        'lte': enddateUTC.isoformat()}}}
...
def esLdapResults(begindateUTC=None, enddateUTC=None):
    ...
    try:
        ...
        q = pyes.FilteredQuery(
            q, pyes.TermFilter('details.result', 'ldap_invalid_credentials'))
        q2 = q.search()
        ...
        stoplist = (
            'o', 'mozilla',
            'dc', 'com', 'mozilla.com', 'mozillafoundation.org', 'org'
        )
        for t in results.facets['details.dn'].terms:
            ...
            qt = pyes.FilteredQuery(
                qt, pyes.TermFilter('details.dn', t['term']))
            qt2 = qt.search()
            qt2.facet.add_term_facet('details.result')
            results = es.search(qt2)
            # sys.stdout.write('{0}\n'.format(results.facets['details.result']
            # .terms))
        ...
        sys.stderr.write(
            'Elastic Search server could not be reached,'
...
def initConfig():
    ...
    options.defaultTimeZone = getConfig(
        'defaulttimezone', 'US/Pacific', options.configfile)
