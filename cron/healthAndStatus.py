...
def main():
    ...
    try:
        ...
        for server in options.mqservers:
            ...
            r = requests.get(
                'http://{0}:{1}/api/queues'.format(server, options.mqapiport),
                auth=auth
            )
            mq = r.json()
            ...
            for m in mq:
                if 'message_stats' in m.keys():
                    ...
                    # print(('\t\t{0} ready {1} unack'.
                    # format(m['messages_ready'],
                    ...
                    if 'deliver_details' in m['message_stats'].keys():
                        ...
                        # print('\t\t{0} in/sec, {1} out/sec'
                        # .format(m['message_stats']['publish_details']['rate']
                        # ,m['message_stats']['deliver_details']['rate']))
                    if 'publish_details' in m['message_stats'].keys():
                        healthlog['details'][m['name']]['publish_eps'] = \
                            m['message_stats']['publish_details']['rate']
                        # print('\t\t{0} in/sec, 0 out/sec'
                        # .format(m['message_stats']['publish_details']
                        # ['rate']))

        # print(json.dumps(healthlog, sort_keys=True, indent=4))
        # post to elastic search servers directly without
        # going through message queues
        es.index(
            index='events',
...
def initConfig():
    ...
    options.mqservers = list(
        getConfig('mqservers', 'localhost', options.configfile).split(','))
