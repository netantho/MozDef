...
def main():
    ...
    try:
        ...
        boto.connect_cloudtrail(aws_access_key_id=options.aws_access_key_id,
            aws_secret_access_key=options.aws_secret_access_key)
        # capture the time we start running so next time we catch
        # any files created while we run.
        lastrun = toUTC(datetime.now()).isoformat()
        # in case we don't archive files..only look at today
        # and yesterday's files.
        ...
        for region in boto.cloudtrail.regions():
            ...
            ct = boto.cloudtrail.connect_to_region(region.name,
                ...
                for bfile in ctbucket.list():
                    ...
                    if 'CloudTrail' in bfile.key and 'json' in bfile.key:
                        if today in bfile.key or yesterday in bfile.key:
                            ...
                            # delete old files so we don't try to keep
                            # reading them.
                            if options.purge:
                                s3file = ctbucket.get_key(bfile)
                                s3file.delete()
...
def initConfig():
    ...
    options.defaultTimeZone = getConfig('defaulttimezone', 'US/Pacific',
        options.configfile)
    # aws credentials to use to connect to cloudtrail
    ...
    options.lastrun = toUTC(getConfig('lastrun',
        toUTC(datetime.now() - timedelta(hours=1)), options.configfile))
