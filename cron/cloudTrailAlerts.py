...
def alertToMessageQueue(alertDict):
    try:
        ...
        # cherry pick items from the alertDict to send
        # to the alerts messageQueue
        ...
        channel.basic_publish(exchange=options.alertexchange,
            routing_key=options.alertqueue, body=json.dumps(mqAlert))
    except Exception as e:
        logger.error('Exception while sending alert to '
                     'message queue: {0}'.format(e))
...
def esCloudTrailSearch(es, begindateUTC=None, enddateUTC=None):
    ...
    try:
        # search for actions within the date range that haven't
        ...
        qEvents = pyes.TermsFilter('eventName',
            ['runinstances', 'stopinstances', 'startinstances'])
        ...
        # results=es.search(pyes.ConstantScoreQuery(pyes.BoolFilter(
        # must=[qcloud,qDate,qEvents])))
        return(results)
    ...
    try:
        if len(esResults) > 0:
            for r in esResults:
                ...
                alert['events'].append(dict(index=r.get_meta()['index'],
                    type=r.get_meta()['type'], id=r.get_meta()['id']))
                alert['eventtimestamp'] = r['eventTime']
                alert['severity'] = 'INFO'
                alert['summary'] = ('{0} called {1} from {2}'.format(
                ...
                if r['eventName'] == 'RunInstances':
                    for i in r['responseElements']['instancesSet']['items']:
                        if 'privateDnsName' in i.keys():
                            alert['summary'] += (' running {0} '.format(
                                i['privateDnsName']))
                ...
                if r['eventName'] == 'StartInstances':
                    for i in r['requestParameters']['instancesSet']['items']:
                        alert['summary'] += (' starting {0} '.format(
                            i['instanceId']))
                ...
                if 'alerts' not in r.keys():
                    r['alerts'] = []
                r['alerts'].append(dict(index=alertResult['_index'],
                    type=alertResult['_type'],
                    id=alertResult['_id']))
                r['alerttimestamp'] = toUTC(datetime.now()).isoformat()
                es.update(r.get_meta()['index'],
                    r.get_meta()['type'],
                    r.get_meta()['id'],
                    document=r)
