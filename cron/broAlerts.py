...
def alertToMessageQueue(alertDict):
    try:
        connection = pika.BlockingConnection(
            pika.ConnectionParameters(host=options.mqserver))
        channel = connection.channel()
        # declare the exchanges
        channel.exchange_declare(exchange=options.alertexchange,
            type='topic')

        # cherry pick items from the alertDict to send to the alerts
        # messageQueue
        mqAlert = dict(severity='INFO',
            category='')
        ...
        channel.basic_publish(
            ...
            body=json.dumps(mqAlert)
        )
    except Exception as e:
        logger.error('Exception while sending alert to message'
                     ' queue: {0}'.format(e))
...
def alertToES(es, alertDict):
    try:
        ...
        logger.error('Elastic Search server could not be reached,'
...
def esSearch(es, begindateUTC=None, enddateUTC=None):
    ...
    try:
        # search for events within the date range that haven't
        # already been alerted (i.e. given an alerttimestamp)
        qDate = pyes.RangeQuery(
            qrange=pyes.ESRange('utctimestamp',
        ...
        pyesresults = es.search(pyes.ConstantScoreQuery(
            pyes.BoolFilter(must=[qType, qDate, qEvents, qdetails, qindicator],
                must_not=[qalerted])), size=1000)
        # uncomment for debugging to recreate alerts for events that already
        # have an alerttimestamp
        ...
        # make a simple list of indicator values that can be
        # counted/summarized by Counter
        resultsIndicators = list()

        # bug in pyes..capture results as raw list or it mutates
        # after first access:
        # copy the hits.hits list as our resusts, which is the same as
        # the official elastic search library returns.
        results = pyesresults._search_raw()['hits']['hits']
        for r in results:
            resultsIndicators.append(r['_source']['details']['seenindicator'])

        # use the list of tuples ('indicator',count) to create a dictionary
        # with:
        ...
        for i in Counter(resultsIndicators).most_common():
            ...
            for r in results:
                if (r['_source']['details']['seenindicator'].encode('ascii',
                 'ignore') == i[0]):
...
def createAlerts(es, indicatorCounts):
    ...
    2) update the events with an alert timestamp so they are not included
    in further alerts
    '''
    try:
        if len(indicatorCounts) > 0:
            for i in indicatorCounts:
                ...
                for e in i['events']:
                    alert['events'].append(dict(
                        index=e['_index'],
                        type=e['_type'],
                        id=e['_id'])
                    )
                alert['severity'] = 'NOTICE'
                alert['summary'] = ('{0} matches for indicator {1} '.format(
                    i['count'], i['indicator']))
                for e in i['events']:
                    # append the relevant events in text format to
                    # avoid errant ES issues.
                    # should be able to just set eventsource to i['events']
                    # but different versions of ES 1.0 complain
                ...
                # save alert to alerts index, update events index with
                # alert ID for cross reference
                ...
                # and update the alerttimestamp on the event itself
                # so it's not re-alerted
                for e in i['events']:
                    if 'alerts' not in e['_source'].keys():
                        e['_source']['alerts'] = []
                    e['_source']['alerts'].append(dict(
                        index=alertResult['_index'],
                        type=alertResult['_type'],
                        id=alertResult['_id'])
                    )
                    e['_source']['alerttimestamp'] = toUTC(
                        datetime.now()).isoformat()
                    es.update(e['_index'], e['_type'], e['_id'],
                        document=e['_source'])
