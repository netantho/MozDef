...
def isTimeToken(token):
    ...
    if any(f(token) for f in (info.jump, info.weekday, info.month, info.hms,
     info.ampm, info.pertain, info.utczone, info.tzoffset)):
        return True
    try:
        float(token)
        return True
...
def splitTime(inString):
    '''
    separate a string into anything that is not a date/time token and
    anything that could be a time token useful for parsing incoming syslog
    to move timestamps into a separate field.
    '''
    outString = []
    outTime = []
...
class eventConsumer(ConsumerMixin):
    ...
    def get_consumers(self, Consumer, channel):
        # return [
        #    Consumer(self.taskQueue, callbacks=[self.on_message],
        #    accept=['json']),
        # ]
        consumer = Consumer(
            self.eventQueue,
        ...
        try:
            ...
            if isinstance(body, dict):
                ...
                try:
                    ...
                    logger.exception("alertworker exception: unknown body"
                                     "type received %r\n" % body)
                    return
            else:
                logger.exception("alertworker exception: unknown body type"
                                 "received %r\n" % body)
                return
            ...
            for rex in options.regexlist:
                if 'tag' in rex.keys():
                    ...
                    if 'tags' not in bodyDict.keys():
                        # no tags to compare
                        return
                    if ('tags' in bodyDict.keys()
                     and rex['tag'] not in bodyDict['tags']):
                        # tags..but not one matching.
                        return
                if ('summary' in bodyDict.keys()
                 and re.findall(rex['expression'], bodyDict['summary'])):
                    ...
                    # events topic queue using the ensure function to shortcut
                    # connection/queue drops/stalls, etc.
                    ensurePublish = self.connection.ensure(self.mqproducer,
                        self.mqproducer.publish, max_retries=10)
                    ensurePublish(bodyDict, exchange=self.alertExchange,
                        routing_key='mozdef.alert')
                    message.ack()
                    break  # only publish once on the first regex hit
        except ValueError as e:
            logger.exception("alertworker exception while processing events"
                " queue %r\n" % e)
...
def initConfig():
    ...
    # do we remove any date string from the 'summary' field
    # (removes syslog timestamps)
    ...
    # regexes={"type":"LDAP Group Update","expression":"ou=groups",
    # "severity":"INFO"}   {"type":"LDAP Delete","expression":"delete",
    # "severity":"INFO"}
    # adding a tag attribute will limit expression matching to items
    # with that tag
    # regexes={"type":"LDAP Group Update","expression":"ou=groups",
    # "severity":"INFO","tag":"ldap"}
