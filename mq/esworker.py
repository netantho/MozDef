...
def keyMapping(aDict):
    ...
    explicitly typed when possible to avoid schema changes for upsteam
    consumers
    ...
    try:
        for k, v in aDict.iteritems():
            ...
            if (removeAt(k.lower()) in ('payload')
             and 'summary' not in aDict.keys()):
                # special case for heka if it sends payload as well
                # as a summary, keep both but move payload to the
                # details section.
                returndict[u'summary'] = toUnicode(v)
            ...
            # nxlog keeps the severity name in syslogseverity,everyone else
            # should use severity or level.
            if (removeAt(k.lower()) in ('syslogseverity',
             'severity', 'severityvalue', 'level')):
            ...
            # nxlog sets sourcename to the processname (i.e. sshd),
            # everyone else should call it process name or pname
            ...
            if (removeAt(k.lower()).startswith('fields.')
             or removeAt(k.lower()).startswith('details.')):
...
def esConnect(conn):
    ...
    return pyes.ES(
        server=(list('{0}'.format(s) for s in options.esservers)),
        bulk_size=options.esbulksize
    )
...
class taskConsumer(ConsumerMixin):
    ...
    def __init__(self, mqConnection, taskQueue, topicExchange, esConnection):
        ...
        if options.esbulksize != 0:
            # if we are bulk posting enable a timer to occasionally flush
            # the pyes bulker even if it's not full
    ...
    def flush_es_bulk(self):
        '''
        if we are bulk posting to elastic search force a bulk post even if
        we don't have enough items to trigger a post normally.
        This allows you to have lots of workers and not wait for events
        for too long if there isn't a steady event stream while still
        retaining the throughput capacity that bulk processing affords.
        '''
        ...
        try:
            ...
            if isinstance(body, dict):
                ...
                try:
                    ...
                    sys.stderr.write("esworker exception: unknown body"
                                     " type received %r\n" % body)
                    message.ack()
                    return
            else:
            ...
            if (normalizedDict is not None
             and isinstance(normalizedDict, dict)
             and normalizedDict.keys()):
                ...
                # figure out what type of document we are indexing and post
                # to the elastic search index.
                doctype = 'event'
                if isCEF(bodyDict):
                ...
                try:
                    ...
                    # handle loss of server or race condition with
                    # index rotation/creation/aliasing
                    try:
                    ...
                    # exception target for queue capacity issues
                    # reported by elastic search so catch the error,
                    # report it and retry the message
                    try:
                        sys.stderr.write('ElasticSearchException: {0} reported'
                                         ' while indexing event'.format(e))
                        message.requeue()
                        return
                    except kombu.exceptions.MessageStateError:
                        # state may be already set.
                        return
                # post the dict (kombu serializes it to json)
                # to the events topic queue
                # using the ensure function to shortcut
                # connection/queue drops/stalls, etc.
                ensurePublish = self.connection.ensure(
                    self.mqproducer,
                ...
                ensurePublish(
                    normalizedDict,
                    exchange=self.topicExchange,
                    routing_key='mozdef.event'
                )
            message.ack()
        except ValueError as e:
            ...
            if not module:
                ...
                if 'message' in dir(module):
                    ...
                    if isinstance(mreg, dict):
                        print('[*] plugin {0} registered to receive messages'
                              ' with {1}'.format(mname, mreg))
                    pluginList.append((mclass, mreg, mpriority))
    return pluginList


def checkPlugins(pluginList, lastPluginCheck):
    updateSinceLastTime = abs(datetime.now() - lastPluginCheck).seconds
    if updateSinceLastTime > options.plugincheckfrequency:
...
def main():
    ...
    # Task Exchange for events sent via http for us to normalize and post
    # to elastic search
    eventTaskExchange = Exchange(
        name=options.taskexchange,
        type='direct',
        durable=True
    )
...
def initConfig():
    ...
    options.defaultTimeZone = getConfig(
        'defaulttimezone',
        'US/Pacific',
        options.configfile
    )
    options.mqserver = getConfig(
        'mqserver',
        'localhost',
        options.configfile
    )
    options.taskexchange = getConfig(
        'taskexchange',
        'eventtask',
        options.configfile
    )
    ...
    # elastic search options. set esbulksize to a non-zero value
    # to enable bulk posting, set timeout to post no matter how many events
    # after X seconds.
    ...
    options.prefetch = getConfig(
        'prefetch',
        50,
        options.configfile
    )
    options.mquser = getConfig(
        'mquser',
        'guest',
        options.configfile
    )
    options.mqpassword = getConfig(
        'mqpassword',
        'guest',
        options.configfile
    )
    options.mqport = getConfig(
        'mqport',
        5672,
        options.configfile
    )
