    ...
    for p, postdata, url in posts:
        try:
            if p.result().status_code >= 500:
                logger.error("exception posting to %s %r"
                             "[will retry]\n" % (url, p.result().status_code))
                # try again later when the next message in forces
                # other attempts at posting.
                logcache.put(postdata)
        except ClosedPoolError as e:
            # logger.fatal("Closed Pool Error exception posting to %s %r %r
            # [will retry]\n"%(url,e,postdata))
            logcache.put(postdata)
        except Exception as e:
            logger.fatal("exception posting to %s %r %r"
                         "[will not retry]\n" % (url, e, postdata))
            sys.exit(1)
...
if __name__ == '__main__':
    ...
    parser.add_option(
        ...
        help="mozdef events URL to use when posting events"
    )
    (options, args) = parser.parse_args()
    sh = logging.StreamHandler(sys.stdout)
    sh.setFormatter(formatter)
    logger.addHandler(sh)
    # create a list of logs we can append json to and call
    # for a post when we want.
    logcache = Queue()
    try:
        for i in range(0, 10):
            ...
            print(i)
            alog = dict(
                eventtime=pytz.timezone('US/Pacific')
