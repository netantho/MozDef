...
cefitemre = re.compile(r'''([a-z,A-Z,0-9]{1,10}=.+?) ''')
# anything between | is a header value
cefheaderre = re.compile(r'''(.+?)\|''')
# cef field keys (something=value)
ceffieldre = re.compile(r'''(\w+)=''')
# anything like field=value
# cefitemre=re.compile(r'''([a-z,A-Z,0-9]{1,10}=.+?)(\w+=)''')
...
class Pygtail(object):
    ...
    def _determine_rotated_logfile(self):
        ...
        for rotated_filename in self._check_rotated_filename_candidates():
            if (exists(rotated_filename)
             and stat(rotated_filename).st_ino == self._offset_file_inode):
    ...
    def _check_rotated_filename_candidates(self):
        ...
        for candidate in glob.glob("%s-[0-9]{8}" % self.filename):
            candidates.append(candidate)
...
def initConfig():
    ...
    options.urls = list(getConfig('urls', 'http://localhost:8080/cef/',
