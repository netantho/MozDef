        ...
        if len(bulkpost) > 10:  # TODO Check for bulk format.
            ...
            for i in eventlist:
                try:
                    ...
                    if 'index' not in json.loads(i).keys():
                        ensurePublish = mqConn.ensure(mqproducer,
                            mqproducer.publish, max_retries=10)
                        ensurePublish(eventDict, exchange=eventTaskExchange,
...
def eventsindex():
    if request.body:
        ...
        ensurePublish = mqConn.ensure(mqproducer, mqproducer.publish,
            max_retries=10)
        ensurePublish(eventDict, exchange=eventTaskExchange,
            routing_key=options.taskexchange)
...
def cefindex():
    if request.body:
        ...
        ensurePublish = mqConn.ensure(mqproducer, mqproducer.publish,
            max_retries=10)
        ensurePublish(cefDict, exchange=eventTaskExchange,
            routing_key=options.taskexchange)
    return
